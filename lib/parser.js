...
const parser = files => {
	...
	let parsedData = [].concat(files).reduce((processedData, srcFile) => {
		
		return _.mergeWith(processedData, parseSrcFile(srcFile), function (a, b) {
...
function parseSrcFile (filePath) {
	
	// Check argument type
	if (!_.isString(filePath)) {
		throw new Error('postCSS parseSrcFile: file path should be a String.');
	}
	
	// Ensure we have access to file by given path.
	try {
	...
	let parsedData = nodeSass.renderSync({
		file: filePath
	});
	
	let compiledTypography = parsedData.css.toString();
	let compiledTypographyData = postCSS.parse(compiledTypography);
	...
	compiledTypographyData.walkRules(rule => {
		...
		let selectorPairs = rule.selector.split(/\s*,\s*/g);
		
		let hasAtRuleParent = rule.parent && rule.parent.type === 'atrule';
		
		let atRuleData = hasAtRuleParent ? {
