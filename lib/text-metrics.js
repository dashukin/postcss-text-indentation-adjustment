...
export default postCSS.plugin('postcss-text-metrics', (options = {}) => {
	...
	const {
		dotReplacement = '%dot%',
		textMetrics = {},
		calculate = true // if corrected values should be calculated
	} = options;
	...
	return scss => {
		...
		scss.each(node => {
			...
			// run processing for comments.
			// run recursive checking for rules.
		...
		function processRule (rule) {
			...
			// "walk/walkRules" processes all descedants, while "each" processes only immediate children
			rule.each(node => {
				...
				if (type === 'rule') {
					...
					// NOTE: higher priority is given to declaration parsing.
					...
					if (node.nodes) {
						// in case if at rule has child nodes (e.g. @media) we should process it like original rule
						processRule(node);
						...
						// e.g. {0, .p1, .l1} from @include adaptive-padding-margin(margin-right, $transactional-panel-map, 0/*{0, .p1, .l1}*/);
		...
		function getPureCorrectedDeclarationValue (declarationValue) {
			...
			correctionGroups.forEach(initialGroup => {
				...
				let initialValue = firstArgument.replace(/\D/g, '');
				let initialUnit = firstArgument.replace(/\d/g, '');
				...
				let correctedValue = correctionDataCollection.map(classNameCorrectionsData => {
					...
					// decrease initial value
					let subtrahend = parseInt(next, 10) || 0;
					return prev - subtrahend;
				}, initialValue);
				
				correctedDeclarationvalue = correctedDeclarationvalue.replace(`{${initialGroup}}`, `${correctedValue}${initialUnit}`);
		...
		function getSelectorBasedCorrectedDeclarationValues (declarationValue) {
			...
			let selectorBasedDeclarations = uniqueSelectors.reduce((declarationsOutput, selector) => {
				...
				correctionGroups.forEach(correctionGroup => {
					...
					let correctedValue = correctionGroupClassNames.reduce((calculatedOutput, correctionClassName) => {
						...
						let correctedClassNameBaseDelta = _.get(correctedClassNameValueData, 'baseDelta', 0);
						let correctedClassNameDecreaseBy = _.get(correctedClassNameValueData, 'decreaseBy', 0);
						
						return calculatedOutput.concat([correctedClassNameBaseDelta, correctedClassNameDecreaseBy]);
					...
					selectorBasedCorrectedDeclarationValue = selectorBasedCorrectedDeclarationValue.replace(`{${correctionGroup}}`, `${correctedValue}${initialUnit}`);
		 ...
		 * @returns {Object[]}
		 */
		function getAtRuleBasedCorrectedDeclarationValues (declarationValue, correctionType = '') {
			...
			let atRuleBasedDeclarations = uniqueAtRules.reduce((declarationsOutput, atRule) => {
				...
				correctionGroups.forEach(correctionGroup => {
					...
					atRuleBasedCorrectedDeclarationValue = atRuleBasedCorrectedDeclarationValue.replace(`{${correctionGroup}}`, `${correctedValue}${initialUnit}`);
		 ...
		 * @returns {Object[]}
		 */
		function getAtRuleSelectorBasedCorrectedDeclarationValues (declarationValue, correctionType = '') {
		...
		function createDeclarations (rule, declarationProperty, declarationValue) {
			...
			declarationValue.forEach(declarationData => {
				...
				if (!selector && !atRule) {
					...
					newRule.selector = `${selector} ${ruleSelector}`;
