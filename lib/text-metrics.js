...
export default postCSS.plugin('postcss-text-metrics', (options = {}) => {
	...
	return css => {
		...
		css.each(node => {
			...
			// as long as we are processing css code - each rule could contain only declaration or comment
			if (node.type === 'rule') {
				processRule(node);
		...
		function processAtRule (atRule) {
			...
			if (atRule.nodes) {
				...
				let atRuleRawValue = _.get(atRule, 'raws.params.raw', '');
		...
		* */
		...
		function getPureCorrectedDeclarationValue (declarationValue) {
			...
			correctionGroups.forEach(initialGroup => {
				...
				let correctionValues = correctionDataCollection.map(classNameCorrectionsData => {
		...
		function getSelectorBasedCorrectedDeclarationValues (declarationValue) {
			...
			let selectorBasedDeclarations = uniqueSelectors.reduce((declarationsOutput, selector) => {
				...
				correctionGroups.forEach(correctionGroup => {
					...
					let correctionValues = correctionGroupClassNames.reduce((calculatedOutput, correctionClassName) => {
		...
		function getCorrectedData (initialValue = 0, ...classNames) {
			...
			classNames.forEach(className => {
				 ...
				 */
				let correctionData = _.get(corrections, className.replace(/\./g, dotReplacement), null);
